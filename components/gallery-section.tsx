...
export function GallerySection() {
  ...
  return (
    <section className="w-full py-12 md:py-24 bg-[#626060] text-white">
      <div className="container px-4 md:px-6">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <Button
                      variant="outline"
                      size="sm"
                      className="bg-white/20 border-white/40 text-white hover:bg-white hover:text-[#626060]"
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogContent className="max-w-4xl bg-white text-[#626060] w-[95vw] max-h-[90vh] overflow-y-auto p-4 md:p-6">
            {selectedEvent && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {selectedEvent.images.length > 1 && (
                      <>
                        <button
                          ...
                          className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-[#626060] p-1 rounded-full"
                        ...
                        <button
                          ...
                          className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-[#626060] p-1 rounded-full"
                      ...
                      {selectedEvent.images.map((image, idx) => (
                        <button
                          ...
                          className={`relative w-16 h-16 flex-shrink-0 rounded-md overflow-hidden transition-all ${
                            currentImageIndex === idx ? "ring-2 ring-[#886C6D]" : "opacity-70 hover:opacity-100"
                          }`}
                ...
                <div className="space-y-4">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-bold text-[#626060]">{selectedEvent.title}</DialogTitle>
                  </DialogHeader>

                  <div className="flex flex-wrap gap-1 -mt-1 mb-3">
                    {selectedEvent.tags.map((tag) => (
                      <Badge key={tag} className="bg-[#886C6D]/20 text-[#886C6D] hover:bg-[#886C6D]/30">
                        {tag}
                      </Badge>
                    ))}
                  </div>

                  <p className="text-[#626060]/80">{selectedEvent.description}</p>

                  <div className="space-y-2 pt-2">
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-[#886C6D]" />
                      <span className="text-sm text-[#626060]/80">{selectedEvent.date}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="h-4 w-4 text-[#886C6D]" />
                      <span className="text-sm text-[#626060]/80">{selectedEvent.location}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4 text-[#886C6D]" />
                      <span className="text-sm text-[#626060]/80">{selectedEvent.attendees} attendees</span>
                    </div>
                  </div>

                  <div className="pt-4">
                    <h4 className="font-medium text-[#626060] mb-2">Services Provided:</h4>
                    <ul className="list-disc pl-5 text-[#626060]/80 space-y-1">
                  ...
                  <div className="pt-4">
                    <Button className="bg-[#886C6D] hover:bg-[#A08888] text-white">Book a Similar Experience</Button>
                  </div>
                </div>
              </div>
            )}
